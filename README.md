...
domain, with typed signatures and short notes. Kept in sync with
the code by a check-time drift gate. Agents may prefer the
shipped `dist/*.d.ts`, which carry the full JSDoc per module.
